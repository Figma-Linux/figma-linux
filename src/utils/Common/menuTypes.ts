...
export const stringOfActionMenuItemName = (name: Menu.Items.MenuName) => {
    switch (name.type) {
        case 'string-key': return name.string;
        case 'plugin-name': return name.plugin;
...
export const isOpenConsole = (data: any) => {
    return data.type === 'open-console'
        && (data.showError === undefined || typeof data.showError === 'string');
}
export const isOpenExtensionDirectory = (data: any) => {
    return data.type === 'open-dir'
        && typeof data.localFileId === 'number';
}

export const isPluginMenuAction = (data: any) => {
    return isRunInstalledPlugin(data)
        || isRunLocalPlugin(data);
}
export const isStaticMenuAction = (data: any) => {
    return isRunLastPlugin(data)
...
export const isMenuAction = (data: any) => {
    return isPluginMenuAction(data)
        || isStaticMenuAction(data);
}

export const isPluginName = (data: any): boolean =>
    data.type === 'plugin-name' && typeof data.plugin === 'string';

export const isActionMenuItemName = (data: Menu.Items.MenuName): boolean =>
    isStringKeyName(data) || isPluginName(data);
...
export const isMenuItem = (data: Menu.MenuItem): boolean => {
    return isActionMenuItem(data)
        || isSeparatorMenuItem(data)
        || isSubmenuMenuItem(data);
};
